...
| Критичность | Файл | Описание | Рекомендация |
|---|---|---|---|
| **Высокая** | `Dockerfile` | **Установка `playwright` браузеров от `root`** — Запуск `playwright install-deps chromium` от `root`, а `playwright install chromium` от `appuser` может вызвать проблемы с правами доступа. | ✅ **ИСПРАВЛЕНО**: Настроен корректный PATH `/home/appuser/.local/bin:$PATH` перед выполнением `playwright install-deps` от root. Системные зависимости устанавливаются с root правами, браузеры - от appuser. |
