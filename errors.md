...
|---|---|---|---|
| **Высокая** | `app/main.py` | **Неограниченный CORS (`allow_origins=["*"]`)**  — Разрешение запросов со всех источников (`*`) создает риски атак типа Cross-Site Request Forgery (CSRF). | ✅ **ИСПРАВЛЕНО v1.11.0**: добавлена переменная `ALLOWED_ORIGINS` (default `*` для совместимости). При wildcard `allow_credentials=False` по требованию спецификации CORS. В prod рекомендуется указать конкретные домены через запятую. |
| **Высокая** | `app/extractors.py` | **Отсутствие проверки SSL-сертификатов при запросах** (`requests` без `verify=True`) — При извлечении данных по URL не проверяется SSL-сертификат, что может привести к атакам "человек посередине" (MITM). | ✅ **ИСПРАВЛЕНО v1.11.0**: во все вызовы `requests.get/head` и `session.get/head` добавлен явный `verify=True`. |
