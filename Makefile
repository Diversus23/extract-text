...
.PHONY: help build dev prod stop logs test test-unit test-integration test-coverage test-legacy clean status lint format lint-check install-linters
...
help: ## 📋 Показать список доступных команд
	...
	@echo "  make dev     - Запустить в режиме разработки (с автоперезагрузкой)"
	@echo "  make prod    - Запустить в продакшене (в фоновом режиме)"
	@echo ""
