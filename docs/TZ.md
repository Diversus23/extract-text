...
**3.10. Обработка веб-страниц (новая функциональность v1.10.0):**
...
* **JavaScript (опционально)**: Возможность выполнения JavaScript для извлечения динамического контента (настраивается через `ENABLE_JAVASCRIPT`, по умолчанию отключено)
