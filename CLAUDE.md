...
CI (`.github/workflows/ci.yml`) запускает `lint` → `test-docker` → matrix на Python 3.10/3.11/3.12 → security → docker build. Линтер блокирующий, MyPy — `continue-on-error`.
